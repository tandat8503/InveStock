      ...
      <div>
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-900">Top sản phẩm trong kỳ</h2>
          ...
          <div className="flex rounded-lg border border-slate-200 bg-slate-50 p-0.5 text-xs font-medium">
            <button
              ...
              className={`rounded-md px-2.5 py-1 transition-all ${
                tab === 'selling' ? 'bg-white text-slate-900 font-semibold shadow-sm' : 'text-slate-500 hover:text-slate-900'
              }`}
            >
              Bán chạy
            </button>
            ...
            items.map((item, idx) => (
              <div key={item.id} className="space-y-1 text-xs">
                <div className="flex items-center justify-between font-medium">
                  <span className="flex items-center gap-2 text-slate-900 truncate max-w-[170px]" title={item.productName}>
                    <span className="flex h-4 w-4 items-center justify-center rounded-full bg-slate-100 font-bold text-[10px] text-slate-500">
                      {idx + 1}
                    </span>
                    {item.productName}
                  </span>
                  <div className="text-right">
                    <span className="font-bold text-slate-900">{formatVND(item.totalValue)}</span>
